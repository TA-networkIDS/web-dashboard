...
const MediumSeverityChart: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData((prevData) => {
        ...
        const newSeverity = Math.floor(Math.random() * 7); // Random severity (0-10)
  ...
  const series = [
    { name: "Medium Severity", data: data.map(d => d.severities)}
  ]
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] sm:px-3 sm:pt-3 pr-3 pb-3 flex flex-col">
      <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Medium Severity</h3>
...
export default MediumSeverityChart;
